"""
FloodLab API - Export Router
Exposes endpoints for GeoJSON, KML, ESRI Shapefile (.zip), GeoTIFF rasters, CSV reports, Decision-Maker PDF, and ZIP Run Packages.
"""
...
def _get_simulation_context(run_id: Optional[str] = None, req: Optional[ExportRequest] = None):
    run = None
    if run_id:
        run = _SIMULATION_STORE.get(run_id) or SIMULATION_STORE.get(run_id)
    if not run and req and req.run_id:
        run = _SIMULATION_STORE.get(req.run_id) or SIMULATION_STORE.get(req.run_id)
...
async def export_csv_by_type(run_id: str, csv_type: str):
    ctx = _get_simulation_context(run_id=run_id)
...
async def export_csv(run_id: str):
    return await export_csv_by_type(run_id, "combined")
