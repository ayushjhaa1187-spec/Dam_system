...
class DatasetValidationError(Exception):
    ...
    def __init__(self, report: ValidationReport):
        ...
        super().__init__(f"Dataset validation failed for scenario '{report.scenario_id}' with {len(report.errors)} error(s):\n - {error_msgs}")
            ...
            try:
                with rasterio.open(dem_path) as src:
                    ...
                    if not src.crs:
                        ...
                        dem_bounds_wgs84 = (min(min_lon, max_lon), min(min_lat, max_lat), max(min_lon, max_lon), max(min_lat, max_lat))
                ...
                errors.append(ValidationIssue(
                    field="inputs.dem",
                    message=f"Failed to open DEM file: {str(e)}",
                    rule_code="DEM_READ_ERROR"
                ))
