...
class ScenarioValidator:
    ...
    def validate_scenario_params(cls, params: Dict[str, Any]) -> ScenarioValidationResult:
        ...
        formation_time = float(params.get("breach_formation_time_hrs", 0.0)
                               ) if params.get("breach_formation_time_hrs") else None
        ...
        frl_msl = float(params.get("full_reservoir_level_frl_msl", 0.0)
                        ) if params.get("full_reservoir_level_frl_msl") else None
        ...
        if dam_height <= 0.0:
            errors.append(ValidationErrorDetail(
                field="dam_height_m",
                message="Dam height must be greater than zero.",
                rule="DAM_HEIGHT_POSITIVE"
            ))
        ...
        return ScenarioValidationResult(
            ...
            warnings=warnings,
            summary=summary,
            checked_rules_count=rules_checked
        )
...
class LayerValidator:
    ...
    def validate_study_boundary_intersection(
        ...
        is_valid = len(errors) == 0
        summary = "Study boundary spatial intersection verified." if is_valid else "Boundary spatial intersection check failed."  # noqa: E501
        return ScenarioValidationResult(
            is_valid=is_valid,
            errors=errors,
            warnings=warnings,
