...
class DamConfig(BaseModel):
    ...
    location: List[float] = Field(
        ..., description="Dam location coordinates [latitude, longitude] in WGS84", min_length=2, max_length=2
    )
    height_m: float = Field(..., gt=0.0, description="Structural dam height in meters")
    storage_volume_mcm: Optional[float] = Field(
        default=None, gt=0.0, description="Reservoir storage volume in Million Cubic Meters (MCM)"
    )
    storage_volume_m3: Optional[float] = Field(
        default=None, gt=0.0, description="Reservoir storage volume in Cubic Meters (m³)"
    )
    hydraulic_head_m: Optional[float] = Field(
        default=None, gt=0.0, description="Hydraulic head at breach initiation in meters"
    )
    crest_length_m: Optional[float] = Field(default=None, gt=0.0, description="Total dam crest length in meters")
    dam_type: Optional[str] = Field(
        default="rockfill", description="Dam construction type (e.g. rockfill, earthfill, concrete_gravity, masonry)"
    )
    crest_elevation_msl: Optional[float] = Field(default=None, description="Crest elevation in meters MSL")
    river_bed_elevation_msl: Optional[float] = Field(
        default=None, description="River bed elevation at dam axis in meters MSL"
    )
...
class BreachConfig(BaseModel):
    ...
    failure_type: str = Field(
        default="overtopping", description="Failure mode: overtopping, piping, instantaneous, or structural_collapse"
    )
    breach_width_m: Optional[float] = Field(default=None, gt=0.0, description="Average breach width in meters")
    breach_formation_time_hr: Optional[float] = Field(
        default=None, gt=0.0, description="Breach development time in hours"
    )
    reservoir_level_pct: Optional[float] = Field(
        default=100.0, gt=0.0, le=150.0, description="Reservoir water level as % of full height"
    )
    breach_model: Optional[str] = Field(
        default="froehlich_2008", description="Empirical formula: froehlich_2008, macdonald, von_thun, or ritter"
    )
...
class RunSettingsConfig(BaseModel):
    ...
    simulation_duration_hr: float = Field(
        default=24.0, gt=0.1, le=168.0, description="Total hydrodynamic simulation duration in hours"
    )
    time_step_s: Optional[float] = Field(
        default=None, gt=0.01, le=120.0, description="Optional fixed or max timestep in seconds"
    )
    cfl: Optional[float] = Field(
        default=0.5, gt=0.05, le=0.95, description="Courant-Friedrichs-Lewy (CFL) stability number"
    )
    target_crs: Optional[str] = Field(
        default=None,
        description="Optional target projected CRS (e.g. EPSG:32643). If null, auto-computed from location.",
    )
    output_interval_s: Optional[float] = Field(
        default=300.0, description="Interval for recording time-series animation frames"
    )
    wet_threshold_m: Optional[float] = Field(
        default=0.10, description="Water depth threshold (m) to classify cell as inundated"
    )
...
class ScenarioConfig(BaseModel):
    ...
    run_settings: RunSettingsConfig = Field(
        default_factory=RunSettingsConfig, description="Numerical simulation controls"
    )
