...
def main(argv=None):
    parser = argparse.ArgumentParser(description="HydroShield Customizable Simulation Framework - Scenario Runner")
    parser.add_argument("--config", "-c", required=True, help="Path to scenario configuration file (JSON or YAML)")
    parser.add_argument(
        "--output", "-o", default=None, help="Output directory to save simulation rasters, vectors, and metrics"
    )
    parser.add_argument("--run-id", default=None, help="Optional custom run ID")
    parser.add_argument("--json", action="store_true", help="Print result payload strictly as JSON")
