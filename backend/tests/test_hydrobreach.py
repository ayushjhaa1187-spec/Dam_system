...
def test_sph_simulation_run():
    config = SPHSimulationConfig(particle_spacing_m=100.0, total_duration_s=600.0, save_interval_s=120.0)
...
def test_delft3d_simulation_run():
    config = Delft3DModelConfig(
        nx=40, ny=16, domain_length_m=10000.0, domain_width_m=800.0, total_duration_s=600.0, save_interval_s=120.0
    )
...
def test_exporters():
    geojson_data = GeospatialExporter.generate_geojson(
        scenario_name="Rishi_Ganga_2021", dam_coords=(30.485, 79.738), reach_length_km=25.0
    )
...
def test_hydrology_engine():
    ...
    inp = HydrologyInput(
        catchment_area_km2=7500.0, curve_number_cn=78.0, rainfall_24h_mm=180.0, time_of_concentration_hrs=6.5
    )
...
def test_uncertainty_engine():
    ...
    inp = UncertaintyInput(preset_id="tehri_dam_bhagirathi", ensemble_size=10)
