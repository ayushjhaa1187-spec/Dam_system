  ...
  useEffect(() => {
    ...
    RESULTS_STATIONS.forEach((st) => {
      ...
      const icon = L.divIcon({
        ...
        html: `
          <div style="
            background: ${isHigh ? '#EF4444' : '#101B33'};
            border: 1.5px solid ${isHigh ? '#FCA5A5' : '#38BDF8'};
            color: #FFFFFF;
            border-radius: 8px;
            padding: 3px 8px;
            font-size: 10px;
            ...
            gap: 4px;
          ">
            <span style="width: 6px; height: 6px; border-radius: 50%; background: ${isHigh ? '#FEF08A' : '#34D399'};"></span>
    ...
    <div className="p-6 sm:p-8 max-w-7xl mx-auto space-y-6 text-hc-ink">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-hc-surface/90 border border-hc-border rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-hc-border">
              ...
              <span className="text-[9px] font-mono text-cyan-400">7 LAYERS</span>
            </div>
            ...
            <div className="space-y-2 text-xs font-medium">
              {[
                ...
                <label
                  key={layer.id}
                  className="flex items-center space-x-2.5 p-2 rounded-xl bg-hc-card/60 hover:bg-hc-card border border-hc-border/80 cursor-pointer transition"
                >
                  <input
                    type="checkbox"
            ...
            <div className="pt-3 border-t border-hc-border space-y-2">
              ...
              <div className="flex justify-between text-[9px] font-mono text-hc-textMuted">
                <span>0 m/s</span>
                <span>5 m/s</span>
                <span>12 m/s</span>
                <span>&gt; 20 m/s</span>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="relative w-full h-[400px] rounded-2xl overflow-hidden border border-hc-border bg-hc-canvas shadow-card-dark">
            <div ref={mapContainerRef} className="absolute inset-0 w-full h-full z-0" />

            <div className="absolute top-3 right-3 z-10 bg-hc-surface/90 backdrop-blur border border-hc-border px-3 py-1.5 rounded-xl text-xs font-mono text-cyan-300 shadow-lg flex items-center gap-1.5">
          ...
          {/* Timeline Scrubber */}
          <div className="bg-hc-surface/90 border border-hc-border rounded-2xl p-4 flex items-center justify-between gap-3">
            <button
            ...
            <div className="flex-1 px-2">
              <div className="flex justify-between items-center text-[10px] font-mono font-bold mb-1">
                ...
                <span className="text-cyan-400">{currentTimeHr.toFixed(1)} hr / 24.0 hr</span>
              </div>
              <input
                type="range"
                min="0"
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="bg-hc-surface/90 border border-hc-border rounded-2xl p-4 space-y-3.5">
            {/* Tabs Header */}
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center text-[11px] font-mono text-hc-textSecondary">
                  ...
                  <div className="flex items-center space-x-2 text-[9px]">
                    <span className="text-cyan-400">&bull; Upstream</span>
                    <span className="text-blue-400">&bull; Midstream</span>
                  </div>
                </div>

                <div className="h-40 bg-hc-canvas rounded-xl border border-hc-border p-2">
                  <svg viewBox="0 0 300 130" className="w-full h-full">
                    {/* Grid */}
                    <line x1="25" y1="20" x2="290" y2="20" stroke="#1E3563" strokeDasharray="2 2" />
                    <line x1="25" y1="65" x2="290" y2="65" stroke="#1E3563" strokeDasharray="2 2" />
                    <line x1="25" y1="110" x2="290" y2="110" stroke="#1E3563" />
            ...
            {activeRightTab === 'statistics' && (
              <div className="space-y-3">
                <div className="flex items-center justify-between text-xs font-bold text-hc-ink">
                  ...
                  <span className="text-cyan-400 font-mono">48.7 km²</span>
                </div>

                <div className="flex items-center justify-between gap-4 p-2 bg-hc-canvas rounded-xl border border-hc-border">
                  {/* SVG Donut Chart */}
                  <svg viewBox="0 0 100 100" className="w-24 h-24 shrink-0">
                    {/* High Risk (Red - 37%) */}
                    <circle cx="50" cy="50" r="35" fill="transparent" stroke="#EF4444" strokeWidth="18" strokeDasharray="81 138" strokeDashoffset="0" />
                    {/* Med Risk (Amber - 36%) */}
                    <circle cx="50" cy="50" r="35" fill="transparent" stroke="#F59E0B" strokeWidth="18" strokeDasharray="79 140" strokeDashoffset="-81" />
                    {/* Low Risk (Green/Blue - 27%) */}
                    <circle cx="50" cy="50" r="35" fill="transparent" stroke="#06B6D4" strokeWidth="18" strokeDasharray="59 160" strokeDashoffset="-160" />
                  </svg>
                  ...
                  <div className="space-y-1.5 text-xs font-mono">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <strong className="text-white">18.2 km²</strong>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                      <span className="flex items-center gap-1 text-amber-400">
                        <span className="w-2 h-2 rounded-full bg-amber-500" /> Med Risk:
                      </span>
                      <strong className="text-white">17.6 km²</strong>
                    </div>
                    <div className="flex items-center justify-between gap-4">
                      <span className="flex items-center gap-1 text-cyan-400">
                        <span className="w-2 h-2 rounded-full bg-cyan-500" /> Low Risk:
                      </span>
                      <strong className="text-white">12.9 km²</strong>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <tbody className="divide-y divide-hc-border/60">
                    {RESULTS_STATIONS.map((st) => (
                      <tr key={st.id}>
                        ...
                        <td className="py-1.5 text-red-400 font-bold">{st.depth}</td>
                        <td className="py-1.5 text-cyan-400">{st.vel}</td>
                        <td className="py-1.5 text-amber-400">{st.arrival}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="pt-2 border-t border-hc-border space-y-1.5">
              <div className="flex items-center justify-between text-[11px] font-mono text-hc-textSecondary">
                ...
                <span className="text-amber-400 font-bold">25,340 PPL</span>
              </div>
              <div className="h-14 bg-gradient-to-r from-blue-950 via-amber-950 to-red-950 rounded-xl border border-hc-border flex items-center justify-center text-[10px] font-mono text-red-200">
      ...
      <div className="bg-hc-surface/90 border border-hc-border rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center space-x-2 text-xs text-hc-textSecondary font-mono">
          ...
          <span>&bull;</span>
          <span>CRS: <strong className="text-cyan-400">EPSG:4326</strong></span>
        </div>

        <div className="flex items-center space-x-3">
          <button
            onClick={() => api.downloadKML(downloadPayload)}
            className="px-4 py-2 rounded-xl bg-hc-card hover:bg-hc-elevated border border-hc-border text-xs font-semibold text-cyan-300 transition flex items-center space-x-1.5"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-xl bg-hc-card hover:bg-hc-elevated border border-hc-border text-xs font-semibold text-emerald-300 transition flex items-center space-x-1.5"
          >
            <Download className="w-3.5 h-3.5" />
