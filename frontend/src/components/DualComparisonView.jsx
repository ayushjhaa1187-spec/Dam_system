    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-hc-surface border border-hc-border rounded-2xl p-8 text-center max-w-2xl mx-auto space-y-4 shadow-2xl">
          ...
          <div className="pt-2">
            <span className="inline-block text-[11px] font-mono bg-hc-bg text-hc-textSecondary px-3 py-1.5 rounded-lg border border-hc-border">
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-center space-x-2 mb-1">
              <SplitSquareVertical className="w-5 h-5 text-hc-active" />
              ...
              <span
                className={`text-xs px-2 py-0.5 rounded-full font-semibold border ${
                  passed
                    ? 'bg-emerald-950 text-hc-success border-emerald-800/60'
          ...
          <div className="text-right">
            <span className="text-[11px] text-hc-textSecondary block">Overall Skill Score</span>
            <span className={`text-2xl font-black tracking-tight ${passed ? 'text-hc-active' : 'text-amber-400'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-4">
          ...
          <p className={`text-[10px] mt-1 ${passed ? 'text-hc-success' : 'text-amber-400'}`}>
        ...
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-4">
          ...
          <p className="text-[10px] text-hc-textSecondary mt-1">Inundation Hit Rate (TP / (TP+FN))</p>
        </div>
        ...
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-4">
          ...
          <p className="text-[10px] text-hc-textSecondary mt-1">False Positives (FP / (TP+FP))</p>
        </div>

        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-4">
          <div className="flex items-center justify-between text-xs text-hc-textSecondary mb-1">
            <span>Mean Absolute Depth Error</span>
            <Layers className="w-4 h-4 text-hc-primary" />
          </div>
          <div className="text-xl font-bold text-hc-ink">
            {formatFinite(metrics.mean_absolute_error_depth_m, 2)} m
          </div>
          <p className="text-[10px] text-hc-textSecondary mt-1">SPH vs Delft3D Δh MAE</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-hc-bg p-3.5 rounded-lg border border-cyan-950">
              <span className="text-[11px] font-bold text-hc-active block mb-1">SPH Solver (Near-Field)</span>
              <div className="space-y-1.5 text-xs text-hc-textSecondary">
                <p>Type: <strong className="text-hc-ink">Lagrangian Mesh-Free</strong></p>
                <p>Peak Surge: <strong className="text-hc-active">{formatFinite(summaryComp.sph?.peak_surge_velocity_ms, 1)} m/s</strong></p>
                <p>Max Inundation: <strong className="text-hc-ink">{formatFinite(summaryComp.sph?.max_inundated_area_km2, 1)} km²</strong></p>
                <p className="text-[11px] text-hc-textSecondary pt-1">
                  Resolves 3D violent dam breach front and fluid momentum.
                </p>
              </div>
            </div>

            <div className="bg-hc-bg p-3.5 rounded-lg border border-blue-950">
              <span className="text-[11px] font-bold text-hc-primary block mb-1">Delft3D FM (Far-Field)</span>
              <div className="space-y-1.5 text-xs text-hc-textSecondary">
                <p>Type: <strong className="text-hc-ink">Eulerian 2D SWE</strong></p>
                <p>Peak Surge: <strong className="text-hc-primary">{formatFinite(summaryComp.delft3d?.peak_surge_velocity_ms, 1)} m/s</strong></p>
                <p>Max Inundation: <strong className="text-hc-ink">{formatFinite(summaryComp.delft3d?.max_inundated_area_km2, 1)} km²</strong></p>
                <p className="text-[11px] text-hc-textSecondary pt-1">
          ...
          {/* Key Findings List */}
          <div className="bg-hc-bg/60 p-3.5 rounded-lg border border-hc-border space-y-2">
            <h4 className="text-xs font-bold text-hc-textSecondary flex items-center space-x-1.5">
              <HelpCircle className="w-3.5 h-3.5 text-hc-active" />
              <span>Key Analytical Insights</span>
            </h4>
            <ul className="text-xs text-hc-textSecondary space-y-1 list-disc list-inside">
        ...
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-hc-border">
            ...
            {frameComps.length > 1 && (
              <div className="flex items-center space-x-2">
                <span className="text-[11px] text-hc-textSecondary">Timestep:</span>
                <select
                  value={selectedFrameIdx}
                  onChange={(e) => setSelectedFrameIdx(Number(e.target.value))}
                  className="bg-hc-bg border border-hc-border text-xs text-hc-active rounded px-2 py-1 focus:outline-none"
                >
          ...
          {activeDiffFrame?.diff_grid && activeDiffFrame.diff_grid.length > 0 ? (
            <div className="space-y-3">
              <div className="bg-hc-bg p-4 rounded-xl border border-hc-border flex flex-col items-center">
                <div
                  className="grid gap-0.5 bg-hc-surface p-1.5 rounded-lg border border-hc-border"
                  style={{
              ...
              {/* Color Scale Legend */}
              <div className="flex items-center justify-between text-[11px] text-hc-textSecondary px-2">
                <div className="flex items-center space-x-1">
                  <div className="w-3 h-3 rounded-xs bg-hc-primary" />
                  <span>Delft3D &gt; SPH (−3m)</span>
                </div>
                <div className="flex items-center space-x-1">
                  <div className="w-3 h-3 rounded-xs bg-hc-border" />
            ...
            <div className="p-8 text-center text-xs text-hc-textSecondary bg-hc-bg rounded-xl border border-hc-border">
      ...
      {Object.keys(gaugeComp).length > 0 && (
        <div className="bg-hc-surface/70 border border-hc-border rounded-xl p-5 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left text-hc-textSecondary">
              <thead className="bg-hc-bg text-hc-textSecondary border-b border-hc-border font-mono">
                <tr>
              ...
              <tbody className="divide-y divide-hc-border/60 font-mono">
                {Object.entries(gaugeComp).map(([gKey, gVal]) => (
                  <tr key={gKey} className="hover:bg-hc-secondary/30">
                    ...
                    <td className="py-2 px-3 text-hc-active">{formatFinite(gVal.sph_peak_depth_m, 1)} m</td>
                    <td className="py-2 px-3 text-hc-primary">{formatFinite(gVal.delft_peak_depth_m, 1)} m</td>
